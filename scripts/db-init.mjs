...
async function syncAllStocks(prod = false, verbose = false) {
  log('执行全量股票同步...', 'step');
...
async function main() {
  console.log('\n' + colors.bold('🗄️  Finspark 数据库初始化工具 v2.0') + '\n');
...
示例:
  # 首次初始化（本地开发）
  node scripts/db-init.mjs --full

  # 仅执行迁移
  node scripts/db-init.mjs --migrate-only

  # 完整初始化 + 全量股票同步
  node scripts/db-init.mjs --full --sync

  # 生产环境迁移
  node scripts/db-init.mjs --migrate-only --prod
`);
    return;
  }
  
  log(`配置: 环境=${prod ? '生产' : '本地'}, 完整=${fullInit}, 重置=${reset}, 同步=${sync}`);
  console.log('');
  ...
  // 全量同步
  if (sync) {
    await syncAllStocks(prod, verbose);
    console.log('');
  }
